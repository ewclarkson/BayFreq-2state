...
function [p12Est, p21Est] = method_of_moments_noflip(dataMul, fTrue, sigmaB1, sigmaB2)
...
%     q = raylcdf(sThresh,sigmaB1); % probability of a single displacement being correctly labelled in the thresholding
    ...
    for idx = 1:Ntraj % do for every trajectory
        ...
        [~,~,n21Est,~] = utilF.calculate_nij(stateVecEst); % observed number of downcrossings
...
%         n12FP = q*(1-q)*(N-1)*(1-q)^lThresh; % assuming long trajectories
        ...
        % n12FP = q*(1-q)*(N-1)/(1-(1-q))^2;
        n12FP = q*(1-q)*(N-1)/(1-2*(1-q));
        % n12FP = q*(1-q)*(N-1);
        n21FP = n12FP;
    ...
    p12Est = sum(n21Vec)/(fTrue*sum(lengthVec));
    p21Est = (fTrue/(1-fTrue))*p12Est;
