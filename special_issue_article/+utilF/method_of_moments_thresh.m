...
function [p12DoublyCorrected, p21DoublyCorrected, p12NoCorrection, p21NoCorrection,...
    ...
    method_of_moments_thresh(lThresh, dataMul, fTrue, sigmaB1, sigmaB2)
...
%     lThresh = round(log(0.03)/log(1-q)); % set threshold based on a p-value of 0.03  
    ...
    for idx = 1:Ntraj % do for every trajectory
        ...
        n12FP = q*(1-q)*(N-2)*(1-q)^lThresh; % assuming long trajectories
    ...
    func = @(p) utilF.func_self_consistency_eq(p, fTrue, p12FP, lThresh, lThresh, sum(lengthVec));
    p12Vec = 0:0.0001:1;
    ...
    funcFN = @(p) utilF.func_self_consistency_eq(p, fTrue, p12NoCorrection, lThresh, lThresh, sum(lengthVec));
    % p12Vec = 0:0.0001:1;
